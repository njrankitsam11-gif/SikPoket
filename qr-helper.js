/**
 * SikPoket Standalone Offline QR Code Generator
 * Minimal, zero-dependency QR Code (Byte mode, ECC Level L/M)
 * Generates sharp canvas QR codes for any URL or text offline.
 */

(function(global) {
  // Minimal QR implementation for URLs up to 300 chars
  // Generates SVG/Canvas representations of QR codes
  
  function createQRCode(text, eccLevel) {
    // Type 1..10 auto-selection
    return QRCodeModel.createData(text, eccLevel || 'M');
  }

  // --- QR Code Model & Polynomial Engine ---
    ...
    get: function() {
      ...
      for (let i = 0; i < this.data.length; i++) {
        ...
        if (c < 0x80) utf8.push(c);
          ...
          i++;
          c = 0x10000 + (((c & 0x3ff) << 10) | (this.data.charCodeAt(i) & 0x3ff));
          utf8.push(0xf0 | (c >> 18));
  ...
  // Helper to auto-calculate minimum QR version
  function getAutoTypeNumber(text, ecc) {
    const len = new QR8bitByte(text).getLength();
    const capacityTable = [
      [14, 11, 7],   // Type 1
      [26, 20, 14],  // Type 2
      [42, 32, 24],  // Type 3
    ...
    for (let t = 0; t < capacityTable.length; t++) {
      if (len <= capacityTable[t][1]) {
        return t + 1;
      }
    }
    return 10;
  ...
  const QRCodeGenerator = {
    renderToCanvas: function(canvas, text, options) {
      ...
      const fgColor = options.fgColor || '#120f0d';
      const bgColor = options.bgColor || '#ffffff';

      const typeNum = getAutoTypeNumber(text, QRErrorCorrectLevel.M);
    ...
    exportNetscapeBookmarks: function(urlsList) {
      let dateNow = Math.floor(Date.now() / 1000);
      let out = `<!DOCTYPE NETSCAPE-Bookmark-file-1>\n<!-- This is an automatically generated file. It will be read and overwritten. DO NOT EDIT! -->\n<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=UTF-8">\n<TITLE>SikPoket Bookmarks</TITLE>\n<H1>Bookmarks</H1>\n<DL><p>\n    <DT><H3 ADD_DATE="${dateNow}" LAST_MODIFIED="${dateNow}" PERSONAL_TOOLBAR_FOLDER="true">SikPoket Bookmarks</H3>\n    <DL><p>\n`;

      (urlsList || []).forEach(item => {
        const title = (item.title || item.url || 'Bookmark').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        const url = (item.url || '').replace(/"/g, '&quot;');
        const tags = (item.tags || []).join(',');
        const addDate = item.createdAt ? Math.floor(item.createdAt / 1000) : dateNow;
        out += `        <DT><A HREF="${url}" ADD_DATE="${addDate}" TAGS="${tags}">${title}</A>\n`;
      });
    ...
    estimateReadingTime: function(text) {
      ...
      return {
        minutes: minutes,
        wordCount: wordCount,
        badgeText: `⏱ ${minutes} min read (${wordCount} words)`
      };
    }
  };
