...
- Loads only 3 helpers (`qr`, `ai`, `audio`) vs. `index.html`'s 12; of those, only `QRCodeGenerator` is actually called — `ai-helper.js`/`audio-helper.js` are loaded but unused there too (ambient-sound buttons exist in markup with no click handlers). Unlike `index.html`, this dead `ai-helper.js` load in `standalone.html` was **not** touched by the 2026-09-02 cleanup — `standalone.html` is a frozen fork, not kept in sync (see below).
...
5. ~~`ai-helper.js` loaded but zero callers here (it's a popup-only feature).~~ **Fixed 2026-09-02** — its dead `<script>` tag removed from `dashboard/index.html` (it remains loaded and used in the popup/side panel). Note: `dashboard/standalone.html` still loads it unused — see § Standalone below, that frozen fork wasn't touched by this cleanup.
