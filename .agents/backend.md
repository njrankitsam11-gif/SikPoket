...
**`api/markdown.js`** (target of `vercel.json`'s `Accept~markdown` rewrites for the 7 negotiated pages) reads `?path=` (or infers from `pathname`, now covering all 7 pages); returns `406` if `Accept` doesn't include `text/markdown`. As explained above, this handler is **effectively unreachable via normal browser/crawler traffic** for all 7 pages, because `proxy.js` always answers first — but it's kept as a defensive fallback and directly-callable endpoint, and is required to keep `vercel.json`'s Accept-header rewrite rules meaningful (`scripts/verify-agentic.js` specifically checks that at least one such rewrite exists). **Fixed 2026-09-02**: both `proxy.js` and `api/markdown.js` now import their `mdMap` from a single shared module, [`lib/markdown-content.js`](../lib/markdown-content.js), instead of each maintaining its own hand-copied literal — this closes the drift risk structurally (previously `api/markdown.js`'s copy was missing 2 of the 7 entries and would have silently wrong-served `/index.md` for `/developers` and `/vercel` if ever reached).
...
1. ~~**Duplicate/conflicting markdown maps.** `proxy.js`'s inline 7-entry `mdMap` always won in production; `vercel.json`'s parallel `Accept~markdown` rewrites to `api/markdown.js` (which had its own incomplete 5-entry map) were dead code for all 7 pages. Editing one map without the other was a real drift risk.~~ **Fixed 2026-09-02** — both files now import the same map from `lib/markdown-content.js`. `proxy.js` still answers first in production (that's unchanged and intentional), but `api/markdown.js` is no longer a second, drifting copy — it's the same data, so it can't go stale relative to `proxy.js` again.
