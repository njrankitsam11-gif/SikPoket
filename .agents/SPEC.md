...
4. `graph-helper.js` physics: `edge.target.target?.mass` typo (should be `edge.target.mass`) causes every edge's target node to get an unscaled force nudge instead of a mass-scaled one. Cosmetic today, real bug if the physics is extended. Tracked in [issue #4](https://github.com/njrankitsam11-gif/SikPoket/issues/4). ([extension.md](extension.md))
